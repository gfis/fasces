...
# Generate systax descriptions of FASS curves
# @(#) $Id$
...
sub genInner { # generate a path description for the "inner S" structure from [1,2,3]
	my ($width) = @_;
	my $width_1 = $width - 1;
	my $letter1 = substr($letters, 0, 1);
	my $result;
	if (1) {
		$result = "[$letter1$width_1+$width_1.1,/$letter1$width_1+$width_1.2,$letter1$width_1+$width_1.3]";
	}
	return $result;
} # genInner
...
sub genOuter { # generate a path description for the "outer meander" structure from [0, inner, 4]
	...
	if ($level == 0) {
		...
		if ($width % 2 == 0) { # even
		$result = "[$letter_1$width_1+$width_1.$x4,/$letter_1$width,$letter_1$width_1+$width_1.$x0]";
		} else {               # odd
		$result = "[/$letter_1$width_1+$width_1.$x4,$letter_1$width,/$letter_1$width_1+$width_1.$x0]";
		}
	} else {
...
__DATA__
georg@nunki:~/work/gits/fasces$ perl src/main/perl/Kn_syntax.pl 5
A0 = []
B0 = []
----
